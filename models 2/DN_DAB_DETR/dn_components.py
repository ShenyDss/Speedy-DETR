...
def tgt_loss_boxes(src_boxes, tgt_boxes, num_tgt,):
    ...
    if len(tgt_boxes) == 0:
        return {
            'tgt_loss_bbox': torch.as_tensor(0.),
            'tgt_loss_giou': torch.as_tensor(0.),
        }
...
def tgt_loss_labels(src_logits_, tgt_labels_, num_tgt, focal_alpha, log=True):
    ...
    if len(tgt_labels_) == 0:
        return {
            'tgt_loss_ce': torch.as_tensor(0.),
            'tgt_class_error': torch.as_tensor(0.),
        }
...
def compute_dn_loss(mask_dict, training, aux_num, focal_alpha):
    ...
    if training and 'output_known_lbs_bboxes' in mask_dict:
        ...
        losses['tgt_loss_bbox'] = torch.as_tensor(0.)
        losses['tgt_loss_giou'] = torch.as_tensor(0.)
        losses['tgt_loss_ce'] = torch.as_tensor(0.)
        losses['tgt_class_error'] = torch.as_tensor(0.)
    ...
    if aux_num:
        for i in range(aux_num):
            ...
            if training and 'output_known_lbs_bboxes' in mask_dict:
                ...
                l_dict = dict()
                l_dict['tgt_loss_bbox'] = torch.as_tensor(0.)
                l_dict['tgt_class_error'] = torch.as_tensor(0.)
                l_dict['tgt_loss_giou'] = torch.as_tensor(0.)
                l_dict['tgt_loss_ce'] = torch.as_tensor(0.)
                l_dict = {k + f'_{i}': v for k, v in l_dict.items()}
                losses.update(l_dict)
    return losses